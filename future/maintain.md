...
（兩檔大小同為 1578 bytes，內容逐行比對一字不差，
  皆以「# **SP 與 DP 的差異**」開頭）
```
**修正：** 待處理 — 兩個方向：一是補上正確的「常見標籤類型」來源內容（若該主題確實要進教材，目前講義與檢核表都沒有引用到它）；二是直接刪除這個重複檔，只留 `sp-dp.md`。屬於用戶的來源資料，不自行刪改，已於對話中回報待決定。檔案已隨 commit `3f7f6bb` 進入版控。
**影響範圍：**
- `Unit_11/resources/常見標籤類型.md` — 內容與 `sp-dp.md` 重複（查證位置：兩檔全文）
